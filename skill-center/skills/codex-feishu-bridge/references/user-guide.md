...
- Bound threads that you continue manually inside Codex Desktop can also emit milestone pushes to Feishu.
- The current implementation is strongest for these desktop-side milestones: task started, analysis updates, file-change application, completion, interruption, and error.
- Feishu-initiated bridge runs still have the most detailed live push coverage because the bridge sees that execution path directly.
