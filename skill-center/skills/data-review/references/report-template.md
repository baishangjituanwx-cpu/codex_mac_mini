...
## 6. 下一批小云雀视频高占比倾向
- 说明：保留这个历史标题是为了兼容现有脚本，但本区块实际服务所有“视频生成”下游任务，不是只服务小云雀
- 主题文案优先：
- 高占比核心母题：
- 高占比核心判断：
- 核心冲突句：
...
- `## 6. 下一批小云雀视频高占比倾向` 虽然保留旧名字，但内容必须先写“视频生成共用结论”，例如主题文案、冲突、场景、动作、开头规则；小云雀或 Seedance 只作为执行补充，不得反过来主导复盘结论。
- 如果本次复盘属于 `*-no-new-batch` 或明确 `无新执行`，`## 7. 未完成核验项` 仍可保留在复盘正文与飞书稿里，但 `## 9. Docker 看板数据映射` 的 `board.summary` 第 4 项和 `board.next` 不得直接搬运这些跨日补抓/补核验待办。
- 对 `*-no-new-batch` 导出的 Docker 看板，`board.summary` 第 4 项应写成 `等待新批次` 一类的中性主动作，`board.next` 只允许写稳定的内容方向或反误判提醒，不允许写 `补24h 对照` 这类跨日操作指令。
