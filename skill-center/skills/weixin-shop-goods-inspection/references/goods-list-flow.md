...
### Blank Table With A Working Official List API

The product micro-app can show `暂无数据` even while the official list API still returns the current rows. In that state, do not combine an old snapshot with a manual increment and do not treat the blank table as zero products.

1. Stay in the same logged-in WeChat merchant session.
2. Instrument `window.fetch` and `XMLHttpRequest` only to observe responses from the official read-only product-list request, currently identified by `scanProductPreview`.
3. Click the visible `销售中` tab and every page number in order so each response is generated by the current official page.
4. In the same execution, retain the response total, page row counts, exact `platformGoodsId`, `platformSkuId`, merchant code, price, stock, and status.
5. Require `displayed total = sum(page row counts) = unique platformGoodsId count`. A mismatch means the full audit is incomplete.
6. Do not persist or report request headers, cookies, tokens, magic values, signatures, or credentials. Persist only the sanitized business response fields needed for the audit.

This recovery path is read-only evidence collection. It does not relax the mutation gate or permit product changes through the browser.
