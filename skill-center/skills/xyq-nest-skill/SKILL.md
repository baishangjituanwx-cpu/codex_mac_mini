...
仓库镜像里的 Windows PowerShell 启动器：

- `{baseDir}/scripts/submit_run.ps1`
- `{baseDir}/scripts/get_thread.ps1`
- `{baseDir}/scripts/upload_file.ps1`
- `{baseDir}/scripts/download_results.ps1`
- 同目录下的 `.cmd` 包装器也可用于 `cmd.exe`

这些 `.ps1` 包装器会优先寻找仓库 `.venv\Scripts\python.exe`、激活中的虚拟环境，或系统里的 `py` / `python` / `python3`。Windows 本地文件路径统一使用带引号的 `C:/...` 绝对路径。
...
```powershell
& "{baseDir}/scripts/submit_run.ps1" --message "生一个动漫视频"
& "{baseDir}/scripts/submit_run.ps1" --message "再生成一个故事视频" --thread-id THREAD_ID
```
...
```powershell
& "{baseDir}/scripts/get_thread.ps1" --thread-id THREAD_ID --run-id RUN_ID --after-seq SEQUENCE
```
...
```powershell
& "{baseDir}/scripts/upload_file.ps1" "C:/Users/alice/Downloads/image.png"
& "{baseDir}/scripts/upload_file.ps1" "C:/Users/alice/Videos/video.mp4"
```
...
```powershell
& "{baseDir}/scripts/download_results.ps1" --urls URL1 URL2 URL3 --output-dir "C:/Users/alice/Downloads/xyq_output" --prefix "artifact"
```
...
Windows 提示：本地文件路径不要写成 `/Users/...`，统一改成 quoted `C:/...` 绝对路径后再传给 `upload_file.ps1`。
...
- Windows 不需要维护单独的 API 行为分支；只需要通过仓库内 `.ps1` 包装器调用同一套 Python 脚本，并坚持使用 quoted `C:/...` 本地路径。
