...
> 文案生成必须先读取当前内容线最近一次已完成的数据复盘；没有新复盘时，要明确写明沿用的是哪一份已验证复盘。
> 还必须显式写出每个平台实际要使用的：视频文件、封面图片文件、标题、文案或正文。不能让执行线程自己猜。
...
- 视频平台发布包必须包含：抖音 `标题+文案`、快手 `标题+文案`、视频号 `短标题+描述`、微博 `标题+配文`、头条号 `标题+描述`
- 如果本批次覆盖 B站，也要同时给出 `标题+简介`
- 如果本批次覆盖图文平台，还要继续产出对应正文包，不能只停在标题矩阵
- Seedance 视频内容包里的头条号默认按视频发布处理；只有明确要求图文派生稿时，才把头条号标记为 `不上传视频`
